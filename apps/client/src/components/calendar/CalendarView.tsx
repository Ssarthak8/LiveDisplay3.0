...
export default function CalendarView({ schedules, rooms, onSelectSlot, isAdmin = false }: CalendarViewProps) {
  ...
  const handleSlotClick = (dateStr: string, hour: number) => {
    if (isAdmin && onSelectSlot && selectedRoomId) {
      ...
      onSelectSlot(dateStr, timeString, selectedRoomId);
    }
  };
                  ...
                  {hoursArray.map((hour, idx) => {
                    ...
                    return (
                      <div
                        ...
                        title={isAdmin ? `Click to schedule at ${hour}:00` : undefined}
                      />
                    );
                  })}
                  ...
                  {schedulesByDate[dateStr]?.map((s) => {
                    ...
                    return (
                      <div
                        key={s._id}
                        className={cn(
                          'absolute left-1.5 right-1.5 rounded-xl border p-2 lg:p-2.5 overflow-hidden transition-all duration-200 hover:z-10 shadow-sm flex flex-col group/card cursor-pointer',
